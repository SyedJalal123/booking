                ...
                @foreach($space_search_fields as $field)
                    ...
                    <div class="col-md-{{ $field['size'] ?? "6" }} border-right">
                        @switch($field['field'])
                            ...
                            @break
                            @case ('seat_type')
                                @include('Space::frontend.layouts.search.fields.seat_type')
                            @break
                            @case ('from_where')
                                @include('Space::frontend.layouts.search.fields.from-where')
                            @break
                            @case ('to_where')
                                @include('Space::frontend.layouts.search.fields.to-where')
                            @break
                        @endswitch
                    </div>
                @endforeach
