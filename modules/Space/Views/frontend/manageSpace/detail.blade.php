...
@section('content')
    <h2 class="title-bar no-border-bottom">
        {{$row->id ? __('Edit: ').$row->title : __('Add new bus')}}
        @if($row->id)
            <div class="title-action">
                <a class="btn btn-info" href="{{route('space.vendor.seat.index',['flight_id'=>$row->id])}}">
                    <i class="fa fa-hand-o-right"></i> {{__("Bus ticket")}}
                </a>
            </div>
        @endif
    </h2>
