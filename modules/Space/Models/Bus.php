...
use Modules\Space\Factories\BusFactory;
...
class Bus extends Bookable
{
    ...
    {
        $meta['seo_title'] = __("Search for Buses");
    ...
    public function getDetailUrl($include_param = true)
    {
        return "#";
        $param = [];
        $urlDetail = app_get_locale(false, false, '/').config('space.space_route_prefix')."/".$this->id;
        if (!empty($param)) {
            ...
            if (!empty($request->flight_seat)) {
                foreach ($request->flight_seat as $flight_seat) {
                    for ($i = 1; $i <= $flight_seat['number']; $i++) {
                        $bookingPassengers = new BusBookingPassengers();
    ...
    public function getPriceInRanges($start_date, $end_date)
    {
        ...
        $datesRaw = $this->BusDateClass::getDatesInRanges($start_date, $end_date, $this->id);
        $dates = [];
        if (!empty($datesRaw)) {
    ...
    {
        $min = BusSeat::select(['price', 'flight_id'])->whereHas('flight', function (Builder $query) {
            $query->where('status', 'publish');
        })->min('price');
        $max = BusSeat::select(['price', 'flight_id'])->whereHas('flight', function (Builder $query) {
            $query->where('status', 'publish');
        })->max('price');
        return [
            $min ?? 0,
    ...
    public function getScoreReview()
    {
        ...
        $list_score = Cache::rememberForever('review_'.$this->type.'_'.$flight_id, function () use ($flight_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $flight_id)->where('object_model', "space")->where("status", "approved")->first();
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Buses", ['number' => $number]);
        }
        return __(":number Bus", ['number' => $number]);
    }
    ...
    public static function search(Request $request)
    {
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            foreach($terms as $term){
                $model_flight->whereHas('terms', function (Builder $builder) use ($term) {
    ...
    static public function getFormSearch()
    {
        ...
        foreach ( $search_fields as &$item){
            if($item['field'] == 'seat_type'){
                $item['seat_types'] = BusSeatType::selectRaw('id,name,code')->get()->toArray();
            }
    ...
    {
        return $this->hasOne(BusStops::class, 'id', 'airport_from')->withDefault();
    }

    public function airportTo()
    {
        return $this->hasOne(BusStops::class, 'id', 'airport_to')->withDefault();
    }

    public function airline()
    {
        return $this->hasOne(BusCompanies::class, 'id', 'airline_id')->withDefault();
    }

    public function flightSeat()
    {
        return $this->hasMany(BusSeat::class, 'flight_id')->orderBy('price');
    }

    public function bookingPassengers()
    {
        return $this->hasMany(BusBookingPassengers::class, 'flight_id')->whereHas('booking', function (Builder $query) {
