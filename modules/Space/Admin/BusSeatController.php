<?php
    ...
    class BusSeatController extends AdminController
    {
        ...
        public function __construct()
        {
            ...
            $this->flight_seat = BusSeat::class;
            $this->flight = Bus::class;
            $this->currentFlight = Bus::class;
        }
        ...
        public function index(Request $request,$flight_id)
        {
            ...
            $data = [
                ...
                'breadcrumbs'        => [
                    [
                        'name'=> __("Bus: :name :code #:id",['name'=>$this->currentFlight->title,'code'=>$this->currentFlight->code,'id'=>$this->currentFlight->id]),
                        'url'  => route('space.admin.edit',$this->currentFlight)
                    ],
                    [
                        'name' => __('Bus seat'),
                        'url'  => route('space.admin.space.seat.index',['flight_id'=>$this->currentFlight->id])
                    ],
                    [
                        'name'  => __('All'),
                        'class' => 'active'
            ...
            return view('Space::admin.space.seat.index', $data);
        }
        public function edit(Request $request,$flight_id, $id)
        {
            ...
            if (empty($row)) {
                return redirect(route('space.admin.space.seat.index'));
            }
            if (!$this->hasPermission('space_manage_others')) {
            ...
            return view('Space::admin.space.seat.detail', $data);
        }
        ...
        public function store( Request $request,$flight_id, $id ){
            ...
            if($id>0){
                ...
                $row = $this->flight_seat::find($id);
                if (empty($row)) {
                    return redirect(route('space.admin.space.seat.index',['flight_id'=>$this->currentFlight->id]));
                }
            ...
            $res = $row->save();
            if ($res) {
                return redirect(route('space.admin.space.seat.edit',['flight_id'=>$flight_id,'id'=>$row->id]))->with('success', __('Bus seat saved') );
            }
        }
